...
training_set_options = ["pa","ny","ny+pa", "de"]
model_options = ['fcn']#, 'unet']
lr_options = [1e-3,1e-4,1e-5]

loss_options = ['qr_forward']
prior_version_options = ['from_cooccurrences_101_15_no_osm_no_buildings']
...
def main():
    ...
    for (states_str, model, lr,loss, prior_version, additive_smooth, prior_smooth) in itertools.product(
        ...
        output_dir = "output/eval/qr_forward_eval"
       ...
       #     + f" experiment.module.output_smooth={additive_smooth}" # use the default because that's what we did in the hp runs...
            ...
            + f" program.log_dir=logs/eval/qr_forward_eval"
            + " trainer.gpus='GPU'"
        )
