    ...
    def __getitem__(self, query: BoundingBox) -> Dict[str, Any]:
        ...
        if len(filepaths) == 0:
            ...
            for layer in self.layers:
                ...
                if layer in [
                    "a_naip", "prior_whole_city_cooccurrences_101_15"
                ]:
...
class EnviroatlasPrior(GeoDataset):
    ...
    * https://doi.org/10.1109/cvpr.2019.01301
    ...
    valid_prior_versions = [f'whole_city_cooccurrences_101_51']
    ...
    def __init__(
        ...
        split: str,
        root: str = "data",
        layers: List[str] = ["a_naip",'p_prior_osm_pa_whole_city_cooccurrences_101_51'],
        transforms: Optional[Callable[[Dict[str, Any]], Dict[str, Any]]] = None,
        cache: bool = True,
        download: bool = False,
        checksum: bool = False,
        ...
        """
        assert split in self.splits
        print([layer in self.valid_layers for layer in layers])
        super().__init__(transforms)  # creates self.index and self.transform
        self.root = root
        self.layers = layers
        self.cache = cache
                ...
                if row["properties"]["split"] == split:
                    ...
                    self.index.insert(
                        ...
                        coords,
                        {
                            ...
                            "e_buildings": row["properties"]["e_buildings"],
                            "h_highres_labels": row["properties"]["h_highres_labels"],
                            "p_prior_osm_whole_city_cooccurrences_101_51": \
                                  row["properties"]["p_prior_osm_whole_city_cooccurrences_101_51"]
                        },
                    )
        ...
        if len(filepaths) == 0:
            ...
            for layer in self.layers:
                ...
                if layer in [
                    ...
                    sample["image"].append(data)
                elif layer in ["p_prior_osm_whole_city_cooccurrences_101_51"]:
                    sample["mask"].append(data)  
                elif layer in ["h_highres_labels"]:
                    # reindex the enviroatlas labels if they're not already 0-10 index
                    if layer == "h_highres_labels" and self.need_toreindex_enviroatlas_labels: 
