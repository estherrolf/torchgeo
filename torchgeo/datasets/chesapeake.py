...
class ChesapeakeCVPRWeakInput(GeoDataset):
    """CVPR 2019 Chesapeake Land Cover dataset.
    ...
    * https://doi.org/10.1109/cvpr.2019.01301
    ...
    added_layers = ["prior_from_cooccurrences_0_0_no_osm_no_buildings",
                    "prior_from_cooccurrences_101_15_no_osm_no_buildings",
                    "prior_from_cooccurrences_101_31_no_osm_no_buildings",
                    "prior_from_cooccurrences_101_51_no_osm_no_buildings",
                    "roads",
    ...
    def __init__(
        ...
        assert np.array([not l.startswith('prior') for l in layers[1:]]).all()
                ...
                if row["properties"]["split"] == split:
                    ...
                    self.index.insert(
                        ...
                        dict({
                            ...
                            "nlcd": row["properties"]["nlcd"],
                            "buildings": row["properties"]["buildings"]
                             }, **dict(zip(self.added_layers,
                                    [row["properties"]["lc"].replace('lc',x) for x in self.added_layers]))
                        )
