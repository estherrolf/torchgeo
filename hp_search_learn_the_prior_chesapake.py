...
training_set_options = ["pa"]
model_options = ["fcn_larger"]
lr_options = [1e-4]#,1e-4,1e-5]#,1e-3]
loss_options = ["nll"]
...
def main():
    ...
    for (states_str, model, lr,loss, nlcd_blur_sigma, additive_smooth) in itertools.product(
        ...
        output_dir = "output/test_learn_prior/"

        command = (
            "python train.py program.overwrite=True config_file=conf/chesapeake_learn_the_prior.yml"
            + f" experiment.name={experiment_name}"
            + f" experiment.module.segmentation_model={model}"
            + f" experiment.module.loss={loss}"
            + f" experiment.module.learning_rate={lr}"
            + f" experiment.module.num_filters=256"
            ...
            + f" program.log_dir=logs/eval2/learn_this_prior"
            + " trainer.gpus=[GPU]"
        )
